...
// 定义获取单个论文成果的函数
async function fetchAchievement(achId) {
    try {
        const cid = await contract.getAchievement(achId);
        console.log(`Achievement ${achId} CID: ${cid}`);
    } catch (error) {
        console.error('Error fetching achievement:', error);
    }
}

fetchAchievement(6); // 替换 1 为具体的成就 ID